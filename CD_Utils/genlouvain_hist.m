...
%         mydisp([num2str(length(unique(y))),' ',num2str(Q)])
    ...
    %save history of S (final node assignments for this "pass")
    S_hist = [S_hist, S];
...
while ~isequal(Sb,S2) %loop around each "pass" (in language of Blondel et al) with B matrix
    ...
    %save history of S (final node assignments for this "pass")
    S_hist = [S_hist, S2]; %warning('adding to S_hist in Bmatrix while loop');
    %S2_hist = [S2_hist, S2];
...
%S_hist = S2_hist; %warning('replaced S_hist with S2_hist');
